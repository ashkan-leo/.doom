...
| Key       | Action                                      |
|-----------+---------------------------------------------|
| =SPC m j= | Open justl buffer (e = exec, w = with args) |
| =SPC m r= | Transient popup: one key per recipe → run   |
...
| Key       | Action                  |
|-----------+-------------------------|
| =SPC o c c= | Open Claude Code menu |
| =SPC o c s= | Start session (project) |

#+begin_src emacs-lisp
(use-package! claude-code-ide
  :defer t
  :config
  ;; Use vterm as the terminal backend (already installed)
  (setq claude-code-ide-terminal-backend 'vterm
        ;; Side window on the right, wide enough for comfortable reading
        claude-code-ide-window-side       'right
        claude-code-ide-window-width      90
        ;; Review Claude's file edits in ediff before applying
        claude-code-ide-use-ide-diff      t)
  ;; Expose Emacs tools (LSP, xref, diagnostics) to Claude via MCP
